...
CREATE TABLE public.events (
  ...
  audience_age_min integer CHECK (audience_age_min >= 0 AND audience_age_min <= 99),
  audience_age_max integer CHECK (audience_age_max >= 0 AND audience_age_max <= 99),
  syllabus text,
  requirements ARRAY DEFAULT '{}'::text[],
  participant_benefits ARRAY DEFAULT '{}'::text[],
...
CREATE TABLE public.image_blobs (
  ...
  metadata jsonb,
  CONSTRAINT image_blobs_pkey PRIMARY KEY (id),
  CONSTRAINT image_blobs_uploaded_by_fkey FOREIGN KEY (uploaded_by) REFERENCES public.users(id)
);
