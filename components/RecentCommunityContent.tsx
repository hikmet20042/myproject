		...
		const load = async () => {
			try {
				setLoading(true);
											...
											const storiesUrl = typeof window === 'undefined'
											  ? `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/stories?page=1&limit=3`
											  : '/api/stories?page=1&limit=3';
				...
				if (storiesRes.ok) {
					...
					if (mounted) {
						...
						setStories(mappedStories);
											...
											const articlesUrl = typeof window === 'undefined'
											  ? `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/articles?page=1&limit=3`
