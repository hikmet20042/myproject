...
interface IUserAnalytics extends mongoose.Document {
  ...
  // Content performance
  mostViewedArticle?: mongoose.Types.ObjectId;
  mostLikedArticle?: mongoose.Types.ObjectId;
...
const UserAnalyticsSchema = new mongoose.Schema<IUserAnalytics>({
  ...
  completedDrafts: { type: Number, default: 0 },
  ...
  avgWordsPerDraft: { type: Number, default: 0 },
  avgWordsPerArticle: { type: Number, default: 0 },
  writingStreak: { type: Number, default: 0 },
  longestStreak: { type: Number, default: 0 },
  lastActiveDate: { type: Date, default: Date.now },
  
  // Content performance
  mostViewedArticle: { type: mongoose.Schema.Types.ObjectId, ref: 'Article' },
  mostLikedArticle: { type: mongoose.Schema.Types.ObjectId, ref: 'Article' },
...
UserAnalyticsSchema.statics.calculateProductivityScore = function(analytics: any) {
  ...
  const contentScore = Math.min(30, (totalArticles + totalStories) * 3);
  const completionScore = Math.min(25, completedDrafts * 2.5);
  ...
  return Math.round(contentScore + completionScore + consistencyScore + engagementScore + volumeScore);
};
