...
export default function ArticleDetailPage({ params }: { params: { id: string } }) {
  ...
  <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-3">{article.title}</h1>
        <div className="text-sm text-gray-500 mb-4">
          ...
          {article.author && (
            <span className="ml-3">
              by {article.authorId ? (
                <Link href={`/profile/${article.authorId}`} className="text-primary hover:underline">{article.author || 'User'}</Link>
              ) : (
                article.author || 'User'
              )}
            </span>
  ...
  <div className="prose max-w-none">
          {showBlockContent ? (
            <BlocknoteReadOnly content={article.contentBlocksJson || article.content} />
          ) : showStringContent ? (
