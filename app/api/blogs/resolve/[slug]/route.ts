...
export async function GET(
  ...
  const { result: rlResult, headers: rlHeaders } = await applyRateLimit({ request, preset: 'publicRead', endpoint: '/api/blogs/resolve/[slug]' })
  if (!rlResult.allowed) {
  ...
  try {
    ...
    return rlh(errorResponse('Bloq həll edilə bilmədi', 'RESOLVE_BLOG_FAILED', {}, 500), rlHeaders)
  }
}
