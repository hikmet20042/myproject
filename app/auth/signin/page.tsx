...
function SignInContent() {
  ...
  const handleCredentialsSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailValid) {
        setError(t('auth.invalidEmailFormat') || 'Invalid email format')
        setLoading(false)
        return
      }
            ...
            <form className="mt-6 space-y-5" onSubmit={handleCredentialsSignIn} autoComplete="off">
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="bg-white rounded-xl shadow-md border-2 border-gray-200 p-1">
                    <div className="grid grid-cols-2 gap-1">
                      ...
                      <button
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, accountType: 'ngo' }))}
                        className={`flex items-center justify-center px-4 py-3 text-sm font-bold rounded-lg transition-all duration-300 ${formData.accountType === 'ngo'
                          ? 'bg-gradient-to-br from-blue-500 to-blue-700 text-white shadow-lg scale-105'
                          : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                          }`}
                      >
